...
def tF(x):
    colorFunc = vtk.vtkLookupTable()
    colorFunc.SetRange(-200,200)
    colorFunc.SetNumberOfColors(256)
    colorFunc.SetHueRange(0.0, 0.667)
    colorFunc.Build()
    col = [0,0,0]
    colorFunc.GetColor(x,col)
    
    opacityFunc = vtk.vtkPiecewiseFunction()
    opacityFunc.AddPoint(-200, 1.0)
    opacityFunc.AddPoint(200, 0.0)
    opacity = opacityFunc.GetValue(0)
